...
@profile
def calculate_global_maxes(grouped: dict[str, list]):
    global_bp_max, global_tp_max, global_lat_max, global_delay_max = 0,0,0,0
    for series in grouped.values():
        for _,_,_,bp,tp,lat_buckets,send_buckets,recv_buckets in series:
    ...
    return global_bp_max, global_tp_max, global_lat_max, global_delay_max
...
def plot(
    ...
    global_bp_max: float,
    global_tp_max: float,
    global_lat_max: float,
    global_delay_max: float
):
...
def main():
    ...
    grouped = group_data(path.Path("output/aggregation"))
    
    global_bp_max, global_tp_max, global_lat_max, global_delay_max = calculate_global_maxes(grouped)
    plot(
        output_dir,
        grouped,
        global_bp_max,
